<?php
/**
 * Load template files of the plugin also include a filter mimo_maps_for_woocommerce_get_template_part<br>
...
function mimo_maps_for_woocommerce_get_template_part( $slug, $name = '', $include = true ) {
	$template = '';
	$path = plugin_dir_path( realpath( dirname( __FILE__ ) ) ) . 'templates/';
	$plugin = Mimo_Maps_For_Woocommerce_Display::get_instance();
	$plugin_slug = $plugin->get_plugin_slug().'/';
	
	// Look in yourtheme/slug-name.php and yourtheme/mimo-maps-for-woocommerce/slug-name.php
	if ( $name ) {
	...
	$template = apply_filters( 'mimo_maps_for_woocommerce_get_template_part', $template, $slug, $name );
